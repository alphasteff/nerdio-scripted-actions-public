...
if ($PeerVnetIds) {
    ...
    foreach ($id in $VnetIds) {
        ...
        ### Set context to peer subscription
        Write-Output "Setting context to subscription $($Resource.SubscriptionId)"
        $context = Set-AzContext -Subscription $Resource.SubscriptionId
        ...
        $InboundPeering = Get-AzVirtualNetworkPeering -VirtualNetworkName $PeerVnet.Name -ResourceGroupName $Resource.ResourceGroupName -ErrorAction SilentlyContinue | Where-Object { $_.RemoteVirtualNetwork.Id -eq $vnet.id }
        if ($InboundPeering) {
        ...
        #### Set context to NME subscription
        Write-Output "Setting context to subscription $NmeSubscriptionId"
        $context = Set-AzContext -Subscription $NmeSubscriptionId
        ...
        $OutboundPeering = Get-AzVirtualNetworkPeering -VirtualNetworkName $vnet.Name -ResourceGroupName $Vnet.ResourceGroupName -ErrorAction SilentlyContinue | Where-Object { $_.RemoteVirtualNetwork.Id -eq $id }
        if ($OutboundPeering) {
